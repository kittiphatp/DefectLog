...
export default async function DefectPage({ params }: DefectPageProps) {
  ...
  const [{ data: defect }, { data: files }] = await Promise.all([
    supabase.from("defects").select("*, users!defects_created_by_fkey(email), assigned_user:users!defects_assigned_to_fkey(email), parent_defect:defects!defects_parent_id_fkey(id, docno)").eq("id", id).single(),
